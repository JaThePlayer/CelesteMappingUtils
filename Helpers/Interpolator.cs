...
internal sealed class Interpolator {
    ...
    public ref struct HandlerU8 {
        ...
        public ReadOnlySpan<byte> ResultNullTerminated()
        {
            if (1 > CapacityLeft)
            {
        ...
        {
            int written;
            while (!Encoding.UTF8.TryGetBytes(data, RemainingSpan(), out written))
            {
                Expand(Data.Length * 2);
            }
            _len += written;
        ...
        public void AppendFormatted(byte[] str)
        {
            AppendLiteral(str.AsSpan());
        }
        
        public void AppendFormatted<T2>(T2 v)
        {
            if (v is IUtf8SpanFormattable)
            {
                int written;
                while (!((IUtf8SpanFormattable)v).TryFormat(RemainingSpan(), out written, "", null))
                {
                    Expand(Data.Length * 2);
                }
                _len += written;
        ...
        public void AppendFormatted<T2>(T2 v, string format)
        {
            if (v is IUtf8SpanFormattable)
            {
                int written;
                while (!((IUtf8SpanFormattable)v).TryFormat(RemainingSpan(), out written, format, null))
                {
                    Expand(Data.Length * 2);
                }
                _len += written;
    ...
    public ref struct Handler {
        ...
        public void AppendFormatted<T2>(T2 v, string format) where T2 : ISpanFormattable
        {
            int written;
            while (!v.TryFormat(RemainingSpan(), out written, format, null))
            {
                Expand(Data.Length * 2);
            }
